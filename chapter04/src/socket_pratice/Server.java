...
public class Server extends JFrame implements ActionListener {
	...
	public Server() {
		init();
		addListener();
		tfPort.requestFocus(); // 이 컴포넌트가 입력 포커스를 받도록함
	}
			...
			public void run() {
				while (true) {
					try {
						...
						UserInformation useInfo = new UserInformation(socket);
						// 각각의 스레드를 등록시켜준다.
						useInfo.start();
					} catch (IOException e) {
	...
	public void broadCast(String str) {
		for (int i = 0; i < vc.size(); i++) {
			UserInformation uinf = vc.elementAt(i); // ArrayList의 get(i)와 동일
			// 여기서 프로토콜의 개념을 사용
	...
	class UserInformation extends Thread {
		...
		public UserInformation(Socket soc) {
		...
		public void run() {
			while (true) {
				try {
					...
					inmessage(msg); // 프로토콜 따라 실행 기능 달라짐(DataInputStream이 입력받음)
				} catch (IOException e) {
					try {
		...
		private void inmessage(String str) {
			StringTokenizer st = new StringTokenizer(str, "/"); // 토큰으로 분할
			...
			if (protocol.equals("Note")) {
				...
				for (int i = 0; i < vc.size(); i++) {
					UserInformation u = vc.elementAt(i);
				...
				for (int i = 0; i < vc_room.size(); i++) {
					RoomInformation r = vc_room.elementAt(i);
	...
	class RoomInformation {
		...
		public RoomInformation(String roomName, UserInformation u) {
		...
		private void roomBroadcast(String str) { // 현재방의 모든 사람들에게 알린다.
			for (int i = 0; i < room_user_vc.size(); i++) {
				UserInformation u = room_user_vc.elementAt(i);
				u.sendMessage(str);
			}
		}

		private void addUser(UserInformation u) {
			room_user_vc.add(u);
		}
		...
		private void removeRoom(UserInformation u) {
